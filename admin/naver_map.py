...
<script>
  ...
  function priorityZone(zone){const bounds=new naver.maps.LatLngBounds(new naver.maps.LatLng(zone.south,zone.west),new naver.maps.LatLng(zone.north,zone.east));const rectangle=new naver.maps.Rectangle({map,bounds,strokeColor:"#7f1d1d",strokeWeight:3,strokeOpacity:1,fillColor:"#ef4444",fillOpacity:.7,clickable:true,zIndex:145});naver.maps.Event.addListener(rectangle,"click",()=>{const anchor=new naver.maps.Marker({position:new naver.maps.LatLng(zone.lat,zone.lng),map:null});openInfo(anchor,'<b style="color:#991b1b">추가 설치 필요지역 #'+zone.rank+'</b><br>위치: '+esc(zone.district||"행정구 확인 불가")+'<br>범죄 고위험영역: '+zone.riskPct.toFixed(1)+'%<br>안전 인프라: '+zone.infraCount+'/3개 ('+zone.infraScore.toFixed(2)+'/5점)<br>CCTV: '+(zone.cctv?"충족":"미충족")+' · 보안등: '+(zone.light?"충족":"미충족")+' · Wi-Fi: '+(zone.wifi?"충족":"미충족")+'<br>최근접 파출소: '+Math.round(zone.policeDistance)+'m<br>'+esc(zone.targetLabel)+' 영향권: '+esc(zone.targetInfluence)+'<br>부족 시설: '+esc(zone.missing)+'<br>선정 이유: '+esc(zone.reason))});return rectangle;}
