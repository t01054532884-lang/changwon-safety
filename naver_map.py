...
<head>
  ...
  <style>
    html,body,#map{width:100%;height:100%;margin:0;padding:0;font-family:Pretendard,"Noto Sans KR",sans-serif}
    body{overflow:hidden;background:#eef2f7}.panel{position:absolute;z-index:1000;background:rgba(255,255,255,.96);border:1px solid #cbd5e1;border-radius:12px;box-shadow:0 4px 18px rgba(15,23,42,.18);color:#0f172a}
...
<body>
  ...
  function initNaverMap(){
    map=new naver.maps.Map("map",{center:new naver.maps.LatLng(35.18,128.62),zoom:10,mapTypeControl:true,zoomControl:true,zoomControlOptions:{position:naver.maps.Position.LEFT_CENTER},scaleControl:true});infoWindow=new naver.maps.InfoWindow({borderWidth:0,backgroundColor:"transparent",anchorSize:new naver.maps.Size(12,8)});
  ...
  window.navermap_authFailure=()=>{document.getElementById("map-error").style.display="flex"};window.initNaverMap=initNaverMap;
  const script=document.createElement("script");script.src="https://oapi.map.naver.com/openapi/v3/maps.js?ncpKeyId=__CLIENT_ID__&callback=initNaverMap";script.async=true;script.onerror=window.navermap_authFailure;document.head.appendChild(script);
  </script>
</body>
</html>
"""
